...
function buildHtmlTree(dir, baseUrl="/uploads", depth = 0){
    ...
    items.forEach(item =>{
        ...
        if (item.isDirectory()){
            out += `<li class="f-name"><strong>${item.name}</strong>${buildHtmlTree(itemPath, urlPath)}</li>`
        }else{
...
<html lang="en">
  ...
  <a href="/">뒤로가기</a>
    ...
    document.querySelectorAll(".f-name").forEach(folder=>{folder.addEventListener("click",(event)=>{
